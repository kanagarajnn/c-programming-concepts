...
#define BITMAP_WORD_SIZE 32  // Each uint32_t covers 32 slots (2^5)
#define BITMAP_WORDS (POOL_SIZE >> 5)  // Optimized division by 32 (2^5)
...
int* allocate(ResourcePool* pool) {
    ...
    int index = (word_index << 5) | bit_index;  // Optimized multiplication
...
void free_resource(ResourcePool* pool, int* resource) {
    ...
    int word_index = index >> 5;  // Optimized division (index / 32)
    int bit_index = index & 31;   // Optimized modulo (index % 32)
